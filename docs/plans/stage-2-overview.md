...
| **2-3** | **Deployed web app** | hosted interactive map | Renders 2-2's District-wide aggregated + equity data. **Reaches the end goal as a complete product** (RANSAC-baseline accuracy). | [detailed](stage-2-part3-plan.md) · **✓ built + [live](https://dhruv-kapri.github.io/Rooftop-Solar-Potential/) 2026-09-08 (ADR-0010)** — preliminary numbers pending calibrated swap (step 6) |
| **2-4** | **Roof-geometry refinement** | classical multi-plane · per-plane POA · obstruction-aware usable area (ML → bounded demo) | Accuracy upgrade behind stable seams (`fit_roof_planes(method="multiplane")` / `usable_area(obstructions=…)`); per-building collapse keeps the downstream contract, so re-propagated by re-running 2-2. | **classical core built 2026-09-08 (steps 1–5, unit-tested)** — city re-run/validation/ML demo (6–8) remain; ADR-0011/0012/0013 |
